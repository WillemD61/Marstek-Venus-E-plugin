...
class MarstekUDPClient:
    ...
    async def _queue_worker(self):
        while True:
            method, params, future, kwargs = await self._command_queue.get()

            if self._command_queue.qsize() > 3:
                _LOGGER.warning("API queue backlog: %d", self._command_queue.qsize())

            try:
                await asyncio.sleep(0.5)   # <-- tune (0.5–1.5 sec)
                result = await self._send_command_internal(method, params, **kwargs)
                # VERY IMPORTANT: throttle device
                await asyncio.sleep(0.5)   # <-- tune (0.5–1.5 sec)
                future.set_result(result)

            except asyncio.CancelledError:
                future.cancel()
                raise   # let the task die cleanly on shutdown
    ...
    async def send_command(self, method, params=None, **kwargs):
        ...
        await self._command_queue.put((method, dict(params) if params else None, future, kwargs))
    ...
    async def _send_command_internal(
        ...
        """Send a command and wait for response (race-condition safe version)."""
        ...
        self._msg_id_counter = (self._msg_id_counter + 1) % 1_000_000
        ...
        _LOGGER.debug("Preparing command %s (id=%s)", method, msg_id)
        ...
        for attempt in range(1, attempt_limit + 1):
            ...
            # --- Create NEW event + response storage per attempt ---
            response_event = asyncio.Event()
            response_data: dict[str, Any] = {}

            def handler(message, addr):
                """Handle incoming response for THIS attempt only."""
                try:
                    ...
                    _LOGGER.error("Response handler error: %s", err, exc_info=True)

            # Register handler for this attempt
            ...
            try:
                ...
                # Let loop process pending UDP packets before sending
                ...
                _LOGGER.debug(
                    "Command %s succeeded in %.2fs (attempt %d)",
                    method,
                    latency,
                    attempt,
                )
                ...
                _LOGGER.warning("Command %s cancelled", method)
                raise
            ...
            if attempt < attempt_limit:
                ...
                _LOGGER.debug("Retrying %s in %.2fs", method, delay)
        ...
        _LOGGER.error("Command %s failed after %d attempts", method, attempt_limit)
        return None
